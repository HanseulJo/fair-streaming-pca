...
class StreamingFairBlockPCA:
    ...
    def evaluate(self, V:np.ndarray=None, R:np.ndarray=None, f:np.ndarray=None, rank=None, mode='vanilla'):
        ...
        if f is not None:
            self.buffer.mu_gap_estim_err.append(f.dot(self.mu_gap/np.linalg.norm(self.mu_gap)))
    ...
    def train(self,
        ...
        for t in pbar:
            ...
            A, X = [], []
            # while min(n_local)<batch_size and not (sum(n_local)>=2*batch_size and min(n_local)>0):
...
def get_args():
    ...
    parser.add_argument('--center_by_mean', type=str, default='local')     # ['local', 'global']
...
def run(args):
    ...
    Algo.train(
        ...
        center_by_mean=args.center_by_mean,
