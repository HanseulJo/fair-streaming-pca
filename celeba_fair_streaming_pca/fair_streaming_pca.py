...
class FairStreamingPCA:
    ...
    def _unfair_subspace_estimation(self,
        ...
        for t in pbar:
            subset = Subset(dataset, range((t-1)*self.block_size_unfair, min(t*self.block_size_unfair, len(dataset))))
            loader = DataLoader(dataset=subset, batch_size=self.batch_size, shuffle=False, num_workers=0, drop_last=False)
            self._unfair_subspace_estimation_with_npm(subset, loader, t)
    ...
    def _unfair_subspace_estimation_with_npm(self, subset, loader, t):
        ...
        _n_iter = (len(subset) + self.batch_size - 1) // self.batch_size
        b_local_group = [0 for _ in range(2)]
        ...
        for batch_index, (img, label) in pbar:
            ...
            x = img.view(img.size(0), self.num_channel, self.d)
            ss = label[...,self.a]
            for s in range(2):
                if self.constraint in ['mean', 'all']:
    ...
    def _principal_component_analysis(self, 
        ...
        for t in trange(1, self.n_iter_pca+1,  desc=f'PCA:'):
            ...
            b_local = 0
            subset = Subset(dataset, range((t-1)*self.block_size_pca, min(t*self.block_size_pca, len(dataset))))
            loader = DataLoader(dataset=subset, batch_size=self.batch_size, shuffle=False, num_workers=0, drop_last=False)
            _n_iter = (len(subset) + self.batch_size - 1) // self.batch_size
    ...
    def fit_offline(self, 
        ...
        for img, label in pbar:
            x = img.view(img.size(0), self.num_channel, -1)  # (batch_size, num_channel, height, width)
            ss = label[...,self.a]
            for s in range(2):
                Xs[s].append(x[ss==s])
        ...
        if constraint in ['mean', 'all']:
            ...
            for c in range(self.num_channel):
                R.append(torch.from_numpy(null_space(XS[c].mT.cpu().numpy())))
